...
(25:25:50 split between control : in vitro blockade : in vivo blockade). 
Is this the intent? Am I missing something?
...
FOR AIM 2, THE PROCESS OF STEPS YOU DESCRIBE IS CORRECT. 
WE WILL JUST END UP WITH LEFTOVER DIALYSATE FROM THE IN VIVO BLOCKADE PROBES.  
```
...
```{r}
power <- 
  data.frame(aim = as.integer(),
             design = as.character(),
             n = as.integer()) %>% 
  rbind(data.frame(aim = 1, design = "One-sample t-test", n = ceiling(L[["aim1"]][["n"]])),
        # data.frame(aim = 2, design = "One-sample t-test", n = ceiling(L[["aim2"]][["n"]])),
        data.frame(aim = 3, design = "One-sample t-test", n = ceiling(L[["aim3"]][["n"]])),
        data.frame(aim = 4, design = "Two-sample t-test", n = ceiling(L[["aim4"]][["n"]] * (1 + dropout)) * 2))
power %>% kable
```
