...
Define a function to simulate power from a mixed model using global simulation parameters.
...
sim <- function (s = 1, n = 10) {
  ...
  V1 <- 9 * (effectSizes ^ 2)  # Random intercept variance; increase to be conservative
...
power <- 
  rbind(data.frame(aim = 1, 
                   ...
                   notes = NA_character_),
        data.frame(aim = 2, 
                   design = "Two-factor, blocked design",
                   model = "Mixed effects model, 2 fixed effects, random effect for subject", 
                   effectSize = NA,
                   n = NA, 
                   notes = NA_character_),
        data.frame(aim = 3, 
